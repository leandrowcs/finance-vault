import { ArrowUpRight, CalendarDays, ChevronDown, ChevronRight, ChevronUp, Circle, CircleCheck, Pencil, ReceiptText, Users, X } from "lucide-react";
...
type IncomeEntry = { id: string; label: string; date: string; amount: number; movement: Movement };
...
type MonthSummary = {
  ...
  relatedExpenseCards: { id: string; label: string; amount: number; date: string; isMovement?: boolean }[];
};
...
function movementBill(movement: Movement, paid: boolean): Bill {
  return { id: movement.id, name: movement.description, owner: movement.owner, amount: movement.amount, due: movement.date, category: movement.category, paid };
}

function MonthDetailsModal({ month, onClose, onToggleBill, onEdit }: { month: MonthSummary; onClose: () => void; onToggleBill: (key: string) => void; onEdit: (movement: Movement) => void }) {
  ...
  return <div className="movement-modal-layer">
    ...
    <section className="movement-modal month-details-modal" role="dialog" aria-modal="true" aria-labelledby="month-details-title">
      ...
      <section className="month-section" aria-labelledby="income-title">
        ...
        <div className="person-grid">{people.map((person) => <section className="person-column" key={person}><div className="column-heading"><div className={`person-avatar ${person.toLowerCase()}`}>{person[0]}</div><div><h3>{person}</h3><span>{currency.format(month.incomeByPerson[person])} no mês</span></div></div>{!hasIncome || month.incomeEntriesByPerson[person].length === 0 ? <div className="empty-state">Nenhuma receita neste mês.</div> : month.incomeEntriesByPerson[person].map((income) => <article className="income-card" key={`${person}-${income.id}`}><div><strong>{currency.format(income.amount)}</strong><span>{income.label}</span></div><time dateTime={income.date}><CalendarDays size={14} />{dateFormatter.format(new Date(`${income.date}T12:00:00`))}</time><button className="entry-edit-button" type="button" aria-label={`Editar receita ${income.label}`} onClick={() => onEdit(income.movement)}><Pencil size={14} /></button></article>)}</section>)}</div>
      </section>
      <section className="month-section expenses-section" aria-labelledby="expenses-title">
        <div className="section-heading"><div><p className="eyebrow">COMPROMISSOS FINANCEIROS</p><h2 id="expenses-title">Despesas do mês</h2></div></div>
...
export function DashboardPage({ movements, greeting, openedDateLabel, daysUntilNextPayment, onToggleBill, isBillPaid, onOpenCalendar, onOpenMovement }: DashboardPageProps) {
  ...
  const months = useMemo(() => {
    return Array.from({ length: 12 }, (_, monthIndex) => {
      ...
      const periods = financePeriods.filter((period) => period.date.startsWith(key));
      const resolvedMovements = movements.map((movement) => ({ ...movement, ...entryOverrides[`movement:${movement.id}`] }));
      const monthMovements = resolvedMovements.filter((movement) => movement.date.startsWith(key));
      ...
      const incomeEntriesByPerson: Record<Person, IncomeEntry[]> = { Leandro: [], Ketlin: [] };
      for (const period of periods) {
        ...
        incomeEntriesByPerson.Leandro.push({ id: `${period.date}-leandro`, label: leandroOverride?.description ?? "Receita recebida", date: leandroOverride?.date ?? period.date, amount: leandroOverride?.amount ?? period.income.leandro, movement: { id: leandroKey, type: "income", amount: leandroOverride?.amount ?? period.income.leandro, date: leandroOverride?.date ?? period.date, description: leandroOverride?.description ?? "Receita recebida", category: leandroOverride?.category ?? "Salário", owner: leandroOverride?.owner ?? "Você" } });
        incomeEntriesByPerson.Ketlin.push({ id: `${period.date}-ketlin`, label: ketlinOverride?.description ?? "Receita recebida", date: ketlinOverride?.date ?? period.date, amount: ketlinOverride?.amount ?? period.income.ketlin, movement: { id: ketlinKey, type: "income", amount: ketlinOverride?.amount ?? period.income.ketlin, date: ketlinOverride?.date ?? period.date, description: ketlinOverride?.description ?? "Receita recebida", category: ketlinOverride?.category ?? "Salário", owner: ketlinOverride?.owner ?? "Esposa" } });
      }
      for (const movement of incomeMovements) {
        if (belongsTo(movement.owner, "Leandro")) {
          incomeEntriesByPerson.Leandro.push({ id: movement.id, label: movement.description, date: movement.date, amount: allocatedAmount(movement.amount, movement.owner), movement });
        }
        if (belongsTo(movement.owner, "Ketlin")) {
          incomeEntriesByPerson.Ketlin.push({ id: movement.id, label: movement.description, date: movement.date, amount: allocatedAmount(movement.amount, movement.owner), movement });
        }
      }
      incomeEntriesByPerson.Leandro.sort((left, right) => left.date.localeCompare(right.date));
      incomeEntriesByPerson.Ketlin.sort((left, right) => left.date.localeCompare(right.date));

      const incomeByPerson: Record<Person, number> = {
        Leandro: incomeEntriesByPerson.Leandro.reduce((total, entry) => total + entry.amount, 0),
        Ketlin: incomeEntriesByPerson.Ketlin.reduce((total, entry) => total + entry.amount, 0),
      };

      const billsByPerson = people.map((person) => {
        const seeded: ExpenseEntry[] = periods.flatMap((period) => period.bills.flatMap((bill) => {
          ...
          const expenseDate = override?.date ? new Date(`${override.date}T12:00:00`) : dueDate(period.date, bill.due);
          return {
            bill: { ...bill, name: override?.description ?? bill.name, amount: override?.amount ?? bill.amount, category: override?.category ?? bill.category, owner, paid: isBillPaid(toggleKey) },
        ...
        const added: ExpenseEntry[] = expenseMovements.filter((movement) => belongsTo(movement.owner, person)).map((movement) => {
      ...
      const expenseTotal = billsByPerson.reduce((total, group) => total + group.bills.reduce((sum, item) => sum + allocatedAmount(item.bill.amount, item.bill.owner), 0), 0);
      const incomeTotal = incomeByPerson.Leandro + incomeByPerson.Ketlin;
      const label = monthLabel(date);
      const relatedExpenseCards = [
        ...periods.map((period) => ({ id: `period-${period.date}`, label: period.label, amount: period.bills.reduce((sum, bill) => sum + bill.amount, 0), date: period.date })),
        ...expenseMovements.map((movement) => ({ id: `movement-${movement.id}`, label: dateFormatter.format(new Date(`${movement.date}T12:00:00`)), amount: movement.amount, date: movement.date, isMovement: true })),
      ];
  ...
  return <div className="page-wrap" id="dashboard">
    ...
    <section className="month-section" aria-labelledby="year-overview-title">
      ...
      <div className="month-accordion-list">{months.map((month) => {
        ...
        return <article className="month-accordion-card" key={month.key}>
          <button className="month-accordion-trigger" type="button" onClick={() => setExpandedMonths((current) => ({ ...current, [month.key]: !current[month.key] }))} aria-expanded={isExpanded}>
            <div><strong>{month.label}</strong><small>{month.incomeTotal > 0 || month.expenseTotal > 0 ? `${currency.format(month.incomeTotal)} em receitas · ${currency.format(month.expenseTotal)} em despesas` : "Sem movimentações"}</small></div>
            <div className="month-accordion-values"><span className={month.balance >= 0 ? "positive" : "negative"}>{currency.format(month.balance)}</span>{isExpanded ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
          </button>
          {isExpanded && <div className="month-accordion-content">
            <div className="month-quick-summary"><article><span>Receitas totais</span><strong>{currency.format(month.incomeTotal)}</strong></article><article><span>Despesas totais</span><strong>{currency.format(month.expenseTotal)}</strong></article><article><span>Balanço do mês</span><strong className={month.balance >= 0 ? "positive" : "negative"}>{currency.format(month.balance)}</strong></article></div>
            <div className="month-person-summary">{people.map((person) => <article key={`${month.key}-${person}`}><h3>{person}</h3><p>Receitas: <strong>{currency.format(month.incomeByPerson[person])}</strong></p><p>Despesas: <strong>{currency.format(month.expenseByPerson[person])}</strong></p></article>)}</div>
            <button className="outline-button month-details-button" type="button" onClick={() => setSelectedMonthKey(month.key)}>Ver todos os detalhes do mês</button>
          </div>}
        </article>;
      })}</div>
    </section>
    ...
    {selectedMonth && <MonthDetailsModal month={selectedMonth} onClose={() => setSelectedMonthKey(null)} onToggleBill={onToggleBill} onEdit={setEditingMovement} />}
    {editingMovement && <MovementModal initialMovement={editingMovement} onClose={() => setEditingMovement(null)} onSubmit={(movement) => { setEntryOverrides((current) => ({ ...current, [movement.id]: movement })); setEditingMovement(null); }} />}
  </div>;
}
