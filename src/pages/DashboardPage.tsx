import { ArrowUpRight, CalendarDays, ChevronDown, ChevronRight, ChevronUp, Circle, CircleCheck, ReceiptText, Users, X } from "lucide-react";
...
type IncomeEntry = { id: string; label: string; date: string; amount: number };
type MonthSummary = {
  key: string;
  label: string;
...
function MonthDetailsModal({ month, onClose, onToggleBill }: { month: MonthSummary; onClose: () => void; onToggleBill: (key: string) => void }) {
...
export function DashboardPage({ movements, greeting, openedDateLabel, daysUntilNextPayment, onToggleBill, isBillPaid, onOpenCalendar, onOpenMovement }: DashboardPageProps) {
  ...
  const months = useMemo(() => {
    return Array.from({ length: 12 }, (_, monthIndex) => {
      ...
      for (const period of periods) {
        incomeEntriesByPerson.Leandro.push({ id: `${period.date}-leandro`, label: "Receita recebida", date: period.date, amount: period.income.leandro });
        incomeEntriesByPerson.Ketlin.push({ id: `${period.date}-ketlin`, label: "Receita recebida", date: period.date, amount: period.income.ketlin });
      }
      for (const movement of incomeMovements) {
        if (belongsTo(movement.owner, "Leandro")) {
          incomeEntriesByPerson.Leandro.push({ id: movement.id, label: movement.description, date: movement.date, amount: allocatedAmount(movement.amount, movement.owner) });
        }
        if (belongsTo(movement.owner, "Ketlin")) {
          incomeEntriesByPerson.Ketlin.push({ id: movement.id, label: movement.description, date: movement.date, amount: allocatedAmount(movement.amount, movement.owner) });
        }
      }
      ...
      const billsByPerson = people.map((person) => {
        const seeded: ExpenseEntry[] = periods.flatMap((period) => period.bills.filter((bill) => belongsTo(bill.owner, person)).map((bill) => {
          const toggleKey = billPeriodKey(period.date, bill.id);
          return {
            bill: { ...bill, paid: isBillPaid(toggleKey) },
            incomeLabel: period.label,
            expenseDate: dueDate(period.date, bill.due),
            toggleKey,
          };
        }));
      ...
      const expenseTotal = periods.reduce((total, period) => total + period.bills.reduce((sum, bill) => sum + bill.amount, 0), 0) + expenseMovements.reduce((total, movement) => total + movement.amount, 0);
  ...
  return <div className="page-wrap" id="dashboard">
    ...
    {selectedMonth && <MonthDetailsModal month={selectedMonth} onClose={() => setSelectedMonthKey(null)} onToggleBill={onToggleBill} />}
  </div>;
}
